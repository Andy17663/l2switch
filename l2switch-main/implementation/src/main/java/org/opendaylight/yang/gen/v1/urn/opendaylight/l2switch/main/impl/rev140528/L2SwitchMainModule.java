...
public class L2SwitchMainModule extends org.opendaylight.yang.gen.v1.urn.opendaylight.l2switch.main.impl.rev140528.AbstractL2SwitchMainModule {
  ...
  public java.lang.AutoCloseable createInstance() {
    ...
    SalRoleService salRoleService = rpcRegistryDependency.getRpcService(SalRoleService.class);
    ...
    if (getIsInstallDropallFlow()) {
      _logger.debug("L2Switch will install a dropall flow on each switch");
      InitialFlowWriter initialFlowWriter = new InitialFlowWriter(salFlowService,salRoleService);
      ...
      _logger.debug("Dropall flows will not be installed");
    }

    if (getIsLearningOnlyMode()) {
      _logger.debug("L2Switch is in Learning Only Mode");
    }
    else {
      // Setup reactive flow writer
      _logger.debug("L2Switch will react to network traffic and install flows");
    ...
    final class CloseResources implements AutoCloseable {
      ...
      public void close() throws Exception {
        ...
        _logger.debug("L2SwitchMain (instance {}) torn down.", this);
      }
    }
    AutoCloseable ret = new CloseResources();
    _logger.debug("L2SwitchMain (instance {}) initialized.", ret);
    return ret;
  }
