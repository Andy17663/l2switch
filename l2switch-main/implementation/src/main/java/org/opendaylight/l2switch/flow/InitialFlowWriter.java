...
public class InitialFlowWriter implements OpendaylightInventoryListener {
  ...
  public InitialFlowWriter(SalFlowService salFlowService, SalRoleService salRoleService) {
    _logger.debug("####### L2 InitialFlowWriter ####### ");
  ...
  private class InitialFlowWriterProcessor implements Runnable {
    ...
    public void addInitialFlows(InstanceIdentifier<Node> nodeId) {
      ...
      //salRoleService
      _logger.debug("Send role request start");
      UpdateControllerRoleInputBuilder input = new UpdateControllerRoleInputBuilder();
      _logger.debug("b1");
      input.setGenerationId(new BigInteger("200"));
      _logger.debug("b2");
      input.setControllerRole(ControllerRoleTypes.MASTER);
      _logger.debug("b3");
      input.setNode(new NodeRef(nodeId));
      _logger.debug("b4");
      Future <RpcResult<UpdateControllerRoleOutput>> roleOutputFuture = salRoleService.updateControllerRole(input.build());
      _logger.debug("b5");
      //salRoleService.updateControllerRole(arg0)
    }
